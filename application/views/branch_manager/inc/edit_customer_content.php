    ...
    <div class="col-12">
        ...
        <div class="card m-b-30">
            <div class="card-body">
                ...
                <form action="<?= base_url() ?>super_admin/update_customer" method="post" enctype="multipart/form-data">


                    <h4 class="mt-0 header-title">Customer Update</h4> <br>

                    <div class="form-group row">
                        <label class="col-sm-2 col-form-label">FILED WORKER</label>
                        <div class="col-sm-6">
                            <select class="form-control" name="fw_id_no">
                                <option>Select</option>
                                <?php foreach ($this->urm->getfield_worker() as $row2) : ?>
                                    <?php $i = 1;
                                    foreach ($this->urm->getonerow_customer() as $row) : ?>
                                        <option <?php if ($row->fw_id_no == $row2->fw_id_no) {
                                                    echo "selected";
                                                } ?> value="<?= $row2->fw_id_no ?>"><?= $row2->field_worker ?>_<?= $row2->branch_code ?></option>
                                    <?php endforeach; ?>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <input type="hidden" name="cm_image" value="<?= $row->cm_image ?>">
                    <input type="hidden" name="cm_nid_front" value="<?= $row->cm_nid_front ?>">
                    <input type="hidden" name="cm_nid_back" value="<?= $row->cm_nid_back ?>">
    ...
    <div class="col-12">
        <div class="card m-b-30">
            <div class="card-body">
                ...
                <a href="<?= base_url() ?>super_admin/customer_list" class="btn btn-danger ml-2">Cancel</a>
                </form>
            </div>
        </div>
    </div>
