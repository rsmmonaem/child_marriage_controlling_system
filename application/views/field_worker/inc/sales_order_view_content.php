    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-body invoice">
                <?php $i = 1;
                foreach ($this->som->getonerow_sales_list() as $row) : ?>
                        ...
                        <div class="col-md-12">
                            <div class="table-responsive">
                                <table class="table mt-4">
                                    ...
                                    <tbody>
                                        <?php $i = 1;
                                        foreach ($this->som->getonerow_sales_details() as $row) : ?>
                                            <tr>
                                                ...
                                                <td><?= $pro_name ?></td>
                                                <td><?= $row->delevery_date ?></td>


                                                <td><?= $row->pro_qnty ?>-<?= $row->measure ?></td>
                                                <form action="<?= base_url() ?>field_worker/sales_order_delevered" method="post" enctype="multipart/form-data">
                                                    <td> <input type="number" name="distribute" value="<?= $row->distribute ?>" <?php if ($row->status == "delevered") {
                                                                                                                                    echo "disabled";
                                                                                                                                } ?>>
                                                    ...
                                                    <input type="hidden" name="sd_id" value="<?= $row->sd_id ?>">
                                                    <input type="hidden" name="pro_id" value="<?= $row->pro_id ?>">
                                                    <input type="hidden" name="sell_price" value="<?= $row->sell_price ?>">
                                                    <input type="hidden" name="order_no" value="<?= $row->order_no ?>">

                                                    <?php if ($row->status != "delevered") { ?>

                                                        <td><input type="submit" name="submit" value="delever"></td>
                                                    <?php } else {
                                                        echo "<td>N/A</td>";
                                                    } ?>
                                                </form>
                    ...
                    <div class="row d-flex justify-content-center">
                        ...
                        <div class="col-lg-12 col-xl-4">
                            <div class="d-print-none float-right">
                                ...
                                <a href="<?= base_url() ?>field_worker/sales_list" class="btn btn-warning text-light">RETURN</a>
