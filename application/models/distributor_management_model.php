<?php
ob_start();
class Distributor_management_model  extends CI_Model {
    ...
    function create_sup_category() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($page_name == "create_distributor") {
                redirect("super_admin/create_distributor/");
            }
            if ($page_name == "create_unit_head") {
                redirect("super_admin/create_unit_head/");
            }
            if ($page_name == "create_common_user") {
                redirect("super_admin/create_common_user/");
            }
        }
    }
    ...
    function create_distributor() {
        ...
        $this->form_validation->set_rules("contact_person", "contact_person", "xss_clean");
        $this->form_validation->set_rules("cont_num", "cont_num", "xss_clean");
        $this->form_validation->set_rules("dis_email", "dis_email", "xss_clean");
        $this->form_validation->set_rules("dis_address", "dis_address", "xss_clean");
        $this->form_validation->set_rules("credit", "credit", "xss_clean");
        $this->form_validation->set_rules("debit", "debit", "xss_clean");

        if ($this->form_validation->run() == FALSE) {
            echo  $this->upload->display_errors();
            $this->load->view('super_admin/create_distributor/error');
        } else {

            $credit = $this->input->post('balance');
            $debit = $this->input->post('debit');
            $balance = $credit - $debit;
            $dis_code = "DIS-" . mt_rand(100, 999);
            ...
            $data = array(
                ...
                'dis_address'                => $this->input->post('dis_address'),
            ...
            $data2 = array(
                ...
                'full_name'         => $this->input->post('distributor'),
                'update_date'         => date('Y-m-d H:i:s')
            );
    ...
    function getonerow_distributor() {
    ...
    function update_distributor() {
        ...
        if ($this->form_validation->run() == FALSE) {
            echo  $this->upload->display_errors();
            $this->load->view('super_admin/distributor_list/error');
        } else {
            ...
            $this->db->where('user_id', $dis_code);
    ...
    function search_distributor() {
        ...
        if ($page_name == "approval_officer") {
            redirect("approval_officer/distributor_list_search/" . $cont_num);
        }
        if ($page_name == "unit_head") {
            redirect("unit_head/distributor_list_search/" . $cont_num);
        }

        if ($page_name == "super_admin") {
            redirect("super_admin/distributor_list_search/" . $cont_num);
        }
    ...
    function search_distributor_result() {
    ...
    function inventory_submit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            foreach ($ivc_query as $ivc_row) {
                ...
                $this->db->where('sup_id', $sup_id);
                $last_bal = $this->db->get("distributor")->row("last_bal");
                $this->db->where('sup_id', $sup_id);
                $present_due = $this->db->get("distributor")->row("due");
                $this->db->where('sup_id', $sup_id);
                $present_commission = $this->db->get("distributor")->row("commission");
                ...
                $this->db->where('sup_id', $sup_id);
                $this->db->update('distributor', $data_distributor_update);
            }
    ...
    function distributor_inventory_list() {
