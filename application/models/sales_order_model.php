...
class Sales_order_model  extends CI_Model {
    ...
    function insert_sales_order() {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('field_worker/create_order/error');
        } else {
            ...
            $pro_name = $this->db->get("distributor_stock")->row("pro_name");
            $this->db->where('pro_id', $pro_id);
            $latest_price = $this->db->get("distributor_stock")->row("latest_price");
            $this->db->where('pro_id', $pro_id);
            $sell_price = $this->db->get("distributor_stock")->row("buy_price");
            $instock = $this->db->get("distributor_stock")->row("instock");
            $pro_qnty = $this->input->post('pro_qnty');
            $total_price = $pro_qnty * $sell_price;
            $this->db->where('pro_id', $pro_id);
            $measure = $this->db->get("distributor_stock")->row("measure");
            ...
            $this->db->insert('sales_cart', $data);
            $page_name = $this->uri->segment(1);
            if ($page_name == "super_admin") {
                redirect("super_admin/create_sales/return/" . $fw_id_no . "/" . $order_no . "/" . $order_date);
            }
            if ($page_name == "field_worker") {
                redirect("field_worker/create_sales/return/" . $fw_id_no . "/" . $order_no . "/" . $order_date);
            }
    ...
    function order_cart_delete() {
        ...
        $this->db->delete('sales_cart');

        $total_cart = $this->db->count_all('sales_cart');
        if ($total_cart >= 1) {
            $page_name = $this->uri->segment(1);
            if ($page_name == "super_admin") {
                redirect("super_admin/create_sales/return/" . $fw_id_no . "/" . $order_no . "/" . $order_date);
            ...
            $page_name = $this->uri->segment(1);
            if ($page_name == "super_admin") {
                redirect("super_admin/create_sales/return/");
            }
            if ($page_name == "field_worker") {
                redirect("field_worker/create_sales/");
            }
        }
    }
    ...
    function sales_order_submit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('field_worker/create_sales/error');
        } else {
            $order_no = $this->input->post('order_no');
            //check invoice no exist or not
            $qry = "SELECT count(*) as cnt from sales_list where order_no= '$order_no'";
            ...
            if ($res[0]->cnt >= 1) {
                ...
                $page_name = $this->uri->segment(1);
                if ($page_name == "super_admin") {
                    redirect("super_admin/create_sales/order_error");
                }
                if ($page_name == "field_worker") {
                    redirect("field_worker/create_sales/order_error");
                }
            }
            ...
            $this->db->insert('sales_list', $data_order);



            $this->db->order_by("so_id", "DESC");
            $this->db->where('order_no', $order_no);
            $or_query = $this->db->get("sales_cart")->result();
            foreach ($or_query as $or_row) {

                $order_no = $or_row->order_no;
                $order_date = $or_row->order_date;
                $delevery_date = $or_row->delevery_date;
                ...
                $this->db->insert('sales_details', $data_or_details);
		...
		$this->db->insert('dis_ledger', $dis_ledger);*/
            ...
            $page_name = $this->uri->segment(1);
            if ($page_name == "super_admin") {
                redirect("super_admin/sales_list/");
            }
            if ($page_name == "field_worker") {
                redirect("field_worker/sales_list/");
            }
        }
    }

    function getsales_list() {
        $this->db->order_by("sl_id", "DESC");
        $query = $this->db->get("sales_list");
        return $query->result();
    }

    function getsales_list_pending() {
        $this->db->order_by("sl_id", "DESC");
    ...
    function getonerow_sales_details() {
        $order_no = $this->uri->segment(3);
        $this->db->where('order_no', $order_no);
    ...
    function sales_order_delevered() {
        $this->load->library("form_validation");
        $this->form_validation->set_rules("distribute", "distribute", "xss_clean");


        if ($this->form_validation->run() == FALSE) {
            $this->load->view('field_worker/sales_order_view/error');
        } else {

            $distribute = $this->input->post('distribute');
            ...
            $update_total_price = $distribute * $sell_price;

            $data_sales_details = array(
            ...
            $this->db->update('sales_details', $data_sales_details);
            ...
            $this->db->where('pro_id', $pro_id);
            $instock = $this->db->get("distributor_stock")->row("instock");
            $stock_update = $instock - $distribute;
            $data_pro_update = array(
            ...
            $this->db->where('order_no', $order_no);
            $commission = $this->db->get("sales_list")->row("commission");
            $this->db->where('order_no', $order_no);
            $payment = $this->db->get("sales_list")->row("payment");
            $sub_total = $intotal - $commission;
            $due = $sub_total - $payment;

            $data_sales_update = array(
            ...
            $this->db->where('order_no', $order_no);
            $this->db->update('sales_list', $data_sales_update);


            redirect("field_worker/sales_order_view/" . $order_no);
        }
    }
