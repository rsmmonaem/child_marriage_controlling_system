...
// Create Notice
    function create_purohit()
    {
        ...
        $this->form_validation->set_rules("purohit_temple_id", "purohit_temple_id", "xss_clean");
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
                ...
                'purohit_temple_id' 		=> $this->input->post('purohit_temple_id'),
    ...
    function get_purohit_home() {
        $this->db->order_by("purohit_id", "DESC");
        $query = $this->db->get("purohit",4);
        return $query->result();
    }
    ...
    function get_purohit_home_id($id)
    {

        $this->db->select('purohit.*, temple.*');
        $this->db->from('purohit');
        $this->db->join('temple', 'purohit.purohit_temple_id = temple.temple_id', 'inner');
        $this->db->where('purohit.purohit_id', $id);
        $query = $this->db->get();
        return $query->row();
    }
